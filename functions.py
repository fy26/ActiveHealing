...
def fgradabs(A, d2x, d2y):
    
    f = (np.abs((A[1:-1, 2:] - A[1:-1, 0:-2]) / d2x) 
                + np.abs((A[2:, 1:-1] - A[0:-2, 1:-1]) / d2y))
...
def timestep_cf(c, cf, df, vfx, vfy, pu, pn1, dx, dy, dxp, dyp, d2x, d2y, dt, X, Y, pei, artv, artv_flag):
    cfn = np.zeros_like(cf)
   
    if artv_flag:
        artv_coeff = pei + artv * cf[1:-1, 1:-1] * np.sqrt(d2x * d2y)**3 * np.abs(laplace(cf, dx, dy, dxp, dyp))
        cfn[1:-1, 1:-1] = (cf[1:-1, 1:-1]
                          - (cf[2:, 1:-1] * vfy[2:, 1:-1] - cf[0:-2, 1:-1] * vfy[0:-2, 1:-1] ) / d2y * dt - 
                           (cf[1:-1, 2:] * vfx[1:-1, 2:] - cf[1:-1, 0:-2] * vfx[1:-1, 0:-2]) / d2x * dt
                           - dt * pn1 * cf[1:-1, 1:-1] * df[1:-1, 1:-1]
                           + dt * pu * c[1:-1, 1:-1] + artv_coeff * dt * laplace(cf, dx, dy, dxp, dyp))
    else:
...
def timestep_c(c, cf, df, vx, vy, pu, pn1, dx, dy, dxp, dyp, d2x, d2y, dt, X, Y, artv, artv_flag):
    cn = np.zeros_like(c)
    
    if artv_flag:
        artv_coeff = artv * c[1:-1, 1:-1] * np.sqrt(d2x * d2y)**3 * np.abs(laplace(c, dx, dy, dxp, dyp))
        cn[1:-1, 1:-1] = (c[1:-1, 1:-1]
                          - (c[2:, 1:-1] * vy[2:, 1:-1] - c[0:-2, 1:-1] * vy[0:-2, 1:-1] ) / d2y * dt - 
                           (c[1:-1, 2:] * vx[1:-1, 2:] - c[1:-1, 0:-2] * vx[1:-1, 0:-2]) / d2x * dt
                           + dt * pn1 * cf[1:-1, 1:-1] * df[1:-1, 1:-1]
                           - dt * pu * c[1:-1, 1:-1] + artv_coeff * dt * laplace(c, dx, dy, dxp, dyp))                           
    else:                       
        cn[1:-1, 1:-1] = (c[1:-1, 1:-1]
                          - (c[2:, 1:-1] * vy[2:, 1:-1] - c[0:-2, 1:-1] * vy[0:-2, 1:-1] ) / d2y * dt - 
                           (c[1:-1, 2:] * vx[1:-1, 2:] - c[1:-1, 0:-2] * vx[1:-1, 0:-2]) / d2x * dt
